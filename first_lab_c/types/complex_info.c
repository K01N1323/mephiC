...
// Складывает два комплексных числа и сохраняет результат в 'out'
...
// Умножает два комплексных числа и сохраняет результат в 'out'
...
// Выводит комплексное число в стандартный поток вывода
...
// Глобальные переменные для одиночки (singleton) информации о комплексном типе
static TypeInfo COMPLEX_TYPEINFO;
int COMPLEX_INIT_TYPEINFO = 0;

// Инициализирует и возвращает структуру TypeInfo для комплексных типов
